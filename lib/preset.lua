...
CubicExpCurve.__index = CubicExpCurve

function CubicExpCurve:new(o)
    setmetatable(o, self)
    for i=0,255 do
        local x = i/255
        o[i] = o:to_exp(x)
...
function CubicExpCurve:to_exp(x)
    return math.exp(self.a * (x ^ 3) + self.b * (x ^ 2) + self.c * x + self.d)
end

function CubicExpCurve:to_lin(y)
    local low = 0
    local high = 255
    while low + 1 < high do
        ...
        if y < self[mid] then
            -- it is in the left.
            high = mid
        elseif y > self[mid] then
            -- it is in the right
            low = mid
        else
            -- shortcut to returning the exact value.
            return mid/255
        end
    end
    local rightness = (y - self[low])/(self[high] - self[low])
    return (low + rightness)/255
end



lpf_curve = CubicExpCurve:new{
    a = -6.59,
    b = 11.6,
    c = 1.9,
    d = 3.68,
...
local function forFilter(unprefix_param, transform)
    local param_name = "doubledecker_" .. unprefix_param
    local param = params:lookup_param(param_name)
    local ret = {}
    ...
    function ret:getByte()
        local val = param:get()
        return util.clamp(util.round(255*transform:to_lin(val), 1), 0, 255)
    end

    function ret:setByte(b)
        param:set(transform:to_exp(b/255))
    end
...
function Preset:init()
    if self.byte_descriptors then return end
    self.byte_descriptors = {
        ...
        { index = 4,  name = "SLIDER_VCF_HPF_A",        converter = forFilter("hp_freq_1", hpf_curve) },
        { index = 5,  name = "SLIDER_VCF_RESH_A",       converter = forParamRaw("hp_res_1") },
        { index = 6,  name = "SLIDER_VCF_LPF_A",        converter = forFilter("lp_freq_1", lpf_curve) },
        ...
        { index = 34, name = "SLIDER_VCF_HPF_B",        converter = forFilter("hp_freq_2", hpf_curve) },
        { index = 35, name = "SLIDER_VCF_RESH_B",       converter = forParamRaw("hp_res_2") },
        { index = 36, name = "SLIDER_VCF_LPF_B",        converter = forFilter("lp_freq_2", lpf_curve) },
        ...
        { index = 58, name = "SLIDER_TR_PITCHBAND",     converter = forParamTransformed("pitch_env", -0.1, 0, 'lin') },
...
function Preset:current_as_bytes()
    self:init()
    local ret = {}
    for _, descriptor in ipairs(self.byte_descriptors) do
...
function Preset:read_file(filename)
    ...
    self.bank = contents
    self.n_psets = math.floor(string.len(self.bank) / 98)
end
